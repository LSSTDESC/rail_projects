...
def test_name_utils() -> None:
    ...
    assert name_utils.get_required_interpolants('xx_{alice}_{bob}') == ['{alice}', '{bob}']
    assert name_utils.format_template('xx_{alice}_{bob}', alice='x', bob='x') == 'xx_x_x'
    ...
    with pytest.raises(ValueError):
        name_utils.resolve_dict(dict(a=('s','d',)), dict(alice='x', bob='y'))
