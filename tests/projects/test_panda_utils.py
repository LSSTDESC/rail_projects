...
class TestUnionDataframesDeduplicated:
    ...
    def test_single_dataframe(self):
        ...
        expected = pd.DataFrame({"id": [1, 2, 3], "value": ["a", "b", "d"]})
    ...
    def test_two_dataframes_with_overlap_keep_first(self):
        ...
        result = union_dataframes_deduplicated(
            [df1, df2], dedup_column="id", keep="first"
        )

        expected = pd.DataFrame({"id": [1, 2, 3, 4], "value": ["a", "b", "c", "z"]})
    ...
    def test_two_dataframes_with_overlap_keep_last(self):
        ...
        result = union_dataframes_deduplicated(
            [df1, df2], dedup_column="id", keep="last"
        )

        expected = pd.DataFrame({"id": [1, 2, 3, 4], "value": ["a", "x", "y", "z"]})
    ...
    def test_three_dataframes_keep_first(self):
        ...
        df2 = pd.DataFrame({"id": [2, 3, 4], "name": ["Bob2", "Charlie2", "David"]})

        df3 = pd.DataFrame({"id": [3, 4, 5], "name": ["Charlie3", "David3", "Eve"]})

        result = union_dataframes_deduplicated(
            [df1, df2, df3], dedup_column="id", keep="first"
        )
    ...
    def test_three_dataframes_keep_last(self):
        ...
        result = union_dataframes_deduplicated(
            [df1, df2, df3], dedup_column="id", keep="last"
        )
    ...
    def test_multiple_columns(self):
        ...
        assert (
            result.loc[result["id"] == 2, "score"].values[0] == 90
        )  # First occurrence
        assert (
            result.loc[result["id"] == 3, "name"].values[0] == "Charlie"
        )  # First occurrence
    ...
    def test_string_dedup_column(self):
        ...
        df2 = pd.DataFrame({"code": ["B", "C", "D"], "value": [20, 30, 40]})

        result = union_dataframes_deduplicated(
            [df1, df2], dedup_column="code", keep="first"
        )

        expected = pd.DataFrame({"code": ["A", "B", "C", "D"], "value": [1, 2, 3, 40]})
    ...
    def test_all_duplicates(self):
        ...
        df3 = pd.DataFrame({"id": [1, 2], "value": ["m", "n"]})
    ...
    def test_preserves_column_order(self):
        ...
        df1 = pd.DataFrame({"z": [1, 2], "a": ["x", "y"], "b": [10, 20]})

        df2 = pd.DataFrame({"z": [3, 4], "a": ["m", "n"], "b": [30, 40]})
    ...
    def test_numeric_dedup_column_with_floats(self):
        ...
        df1 = pd.DataFrame({"id": [1.0, 2.5, 3.7], "value": ["a", "b", "c"]})

        df2 = pd.DataFrame({"id": [2.5, 3.7, 4.2], "value": ["x", "y", "z"]})
        ...
        expected = pd.DataFrame(
            {"id": [1.0, 2.5, 3.7, 4.2], "value": ["a", "x", "y", "z"]}
        )
    ...
    def test_reset_index_properly(self):
        ...
        df1 = pd.DataFrame(
            {"id": [5, 10, 15], "value": ["a", "b", "c"]}, index=[100, 200, 300]
        )

        df2 = pd.DataFrame({"id": [20, 25], "value": ["d", "e"]}, index=[400, 500])
