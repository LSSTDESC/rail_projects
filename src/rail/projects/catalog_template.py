...
class RailProjectCatalogInstance(Configurable):
    ...
    config_options: dict[str, StageParameter] = dict(
        ...
        iteration_vars=StageParameter(
            list, [], fmt="%s", msg="Variables to iterate over to construct catalog",
        ),
    )
    ...
    def __call__(self, **kwargs) -> list[str]:
        ...
        iterations = itertools.product(
            *[kwargs.get(key, []) for key in kwargs.keys()]
        )
    ...
    def check_files(self, **kwargs) -> list[bool]:
...
class RailProjectCatalogTemplate(Configurable):
    ...
    def make_catalog_instance(self, name: str, **kwargs) -> RailProjectCatalogInstance:

        iteration_var_dict = {key: "{"+key+"}" for key in self.config.iteration_vars}        
        formatted_path = self.config.path_template.format(**kwargs, **iteration_var_dict)
